        ...
        <div class="row">
          <div class="col-lg-12">
            <div class="card">
              <div class="card-body">
                <div class="mb-2">
                  <button id="btnTambah" class="btn btn-sm btn-success rounded">&plus; Tambah Outlet</button>
                </div>
            ...
            <div class="row">
              <div class="col-12">
                <div class="form-group">
                  <label for="inputTambahNama">Nama Outlet</label>
          ...
          <div class="m-3">
            <div class="row">
              <div class="col-12">
                ...
                <div class="form-group">
                  <label for="inputEditNama">Nama Outlet</label>
  ...
  $(document).ready(function(){
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          for (let a = 0; a < data.length; a++) {
              let attrs = " data-id='"+ data[a].id_outlet +"' data-nama='"+ data[a].nama_outlet +"' data-alamat='"+ data[a].alamat +"' data-telp='"+data[a].telp+"' ";
              html += `<tr>
                <th scope="row">`+(a+1)+`</th>
                <td>`+data[a].nama_outlet+`</td>
                <td>`+data[a].alamat+`</td>
                <td>`+data[a].telp+`</td>
                <td><button id="btnEdit" `+ attrs +` class="btn btn-primary btn-xs"><i class="nav-icon fas fa-edit"></i></button> <button id="btnHapusOutlet" class="btn btn-danger btn-xs" data-id="`+data[a].id_outlet+`"><i class="nav-icon fas fa-trash"></i></button></td>
              </tr>`
          }
          $('#tbodyOutlet').html(html);
        }
      })
    }
    ...
    $("#btnSimpanTambah").on("click", function(){
      ...
      if (nama != "" && telp != "" && alamat != ""){
        $.ajax({
          type: 'POST',
          url: '<?php echo base_url() ?>outlet/createoutlet',
          dataType: 'json',
          data: {
            nama,
            alamat,
    ...
    $(document).on("click", "#btnSimpanEdit", function(){
      ...
      if (id != "" && nama != "" && telp != "" && alamat != ""){
        $.ajax({
          type: 'POST',
          url: '<?php echo base_url() ?>outlet/updateoutlet',
          dataType: 'json',
          data: {
            id,
            nama,
    ...
    $(document).on("click", "#btnHapusOutlet", function(){
      let id = $(this).data("id")
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            type: 'POST',
            url: '<?php echo base_url() ?>outlet/deleteoutlet',
            dataType: 'json',
            data: {
              id
            },
